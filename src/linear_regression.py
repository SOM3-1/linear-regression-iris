...
def gradient_descent(Xb, y, alpha=0.05, iterations=10000):
...
def cross_validate_kfold(X, y, k=5, seed=42, alpha=0.05, iterations=10000):
    folds = stratified_kfold_indices(y, k=k, seed=seed)
    accs = []
    costs = []
    all_idx = np.arange(X.shape[0], dtype=int)
    ...
    for i in range(k):
        ...
        X_val_n = minmax_transform(X_val, x_min, denom)

        Xb_train = add_bias_column(X_train_n)
        Xb_val = add_bias_column(X_val_n)

        w, cost_history = gradient_descent(Xb_train, y_train, alpha=alpha, iterations=iterations)

        y_hat_val = predict(Xb_val, w)
        y_pred_val = nearest_label(y_hat_val)
        accs.append(accuracy(y_pred_val, y_val))
        costs.append(cost_history[-1])

    return np.array(accs, dtype=float), np.array(costs, dtype=float)

def main():
    ensure_dataset()
    X, y = load_iris(DATA_PATH)

    if X.ndim != 2 or X.shape[1] != 4 or y.ndim != 1 or X.shape[0] != y.shape[0] or X.shape[0] == 0:
        raise RuntimeError(f"Invalid dataset shape: X={X.shape}, y={y.shape}")

    cv_accs, cv_costs = cross_validate_kfold(
        X, y, k=5, seed=42, alpha=0.05, iterations=10000)

    print("\n=== 5-Fold Cross-Validation Summary ===")
    print(f"{'Fold':<10}{'Accuracy (%)':>15}{'Final Cost':>15}")
    print("-" * 40)
    for i in range(5):
        print(f"{i+1:<10}{cv_accs[i]*100:>15.2f}{cv_costs[i]:>15.6f}")
    print("-" * 40)
    print(f"{'Mean':<10}{cv_accs.mean()*100:>15.2f}{cv_costs.mean():>15.6f}")
    print(f"{'Std':<10}{cv_accs.std()*100:>15.2f}{cv_costs.std():>15.6f}")
    print('')

    os.makedirs(IMG_DIR, exist_ok=True)
    petal_scatter_graph(X, y, SCATTER_GRAPH)

    X_train, y_train, X_test, y_test = stratified_split_120_30(X, y, seed=123)
    ...
    w, cost_history = gradient_descent(Xb_train, y_train, alpha=0.05, iterations=10000)
    ...
    print("Saved graphs in images folder")
    print('')
